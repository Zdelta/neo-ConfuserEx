...
namespace dnlib.DotNet
{
    ...
    public abstract class TypeDef : ITypeDefOrRef, IHasCustomAttribute, IHasDeclSecurity, IMemberRefParent, ITypeOrMethodDef, IListListener<FieldDef>, IListListener<MethodDef>, IListListener<TypeDef>, IListListener<EventDef>, IListListener<PropertyDef>, IListListener<GenericParam>, IMemberRefResolver, IMemberDef
    {
        ...
        public MethodDef FindOrCreateInstanceConstructor()
        {
            var cctor = FindDefaultConstructor();
            if (cctor != null)
                return cctor;

            var implFlags = MethodImplAttributes.IL | MethodImplAttributes.Managed;
            var flags = MethodAttributes.Public | MethodAttributes.SpecialName | MethodAttributes.RTSpecialName;
            var module = Module;
            cctor = module.UpdateRowId(new MethodDefUser(MethodDef.InstanceConstructorName,
            ...
            return Methods.ExecuteLocked<MethodDef, MethodDef, MethodDef>(cctor, (tsList, cctor2) =>
            {
                var cctor3 = FindInstanceConstructor_NoMethodsLock();
                if (cctor3 != null)
                    return cctor3;
                tsList.Add_NoLock(cctor2);
                return cctor2;
        ...
        private MethodDef FindInstanceConstructor_NoMethodsLock()
        {
            foreach (var method in Methods.GetEnumerable_NoLock())
            {
